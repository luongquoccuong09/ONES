...
namespace ONES
{
    ...
    public class ExportCeilingLayers : IExternalCommand
    {
        // Token: 0x06000380 RID: 896 RVA: 0x00037644 File Offset: 0x00035844
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            stopwatch.Start();
            FilteredElementCollector source = new FilteredElementCollector(document).WhereElementIsElementType().OfCategory((BuiltInCategory)(-2000038));
            List<CeilingType> list = (
                from x in source.OfType<CeilingType>()orderby x.Name
                select x).ToList<CeilingType>();
            Worksheet worksheet = Utils.ExcelWorksheet("Ceilings");
            ...
            int num = 2;
            foreach (CeilingType ceilingType in list)
            {
                if (ceilingType.GetCompoundStructure() != null)
                {
                    try
                    {
                        worksheet.Cells[num, 1] = ceilingType.Name;
                    }
                    catch (Exception)
                    {
