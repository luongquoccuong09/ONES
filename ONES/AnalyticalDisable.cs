...
namespace ONES
{
    ...
    public class AnalyticalDisable : IExternalCommand
    {
        // Token: 0x060000CA RID: 202 RVA: 0x00012D14 File Offset: 0x00010F14
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            FilterIntegerRule filterIntegerRule = new FilterIntegerRule(parameterValueProvider, new FilterNumericEquals(), 1);
            ...
            using (Transaction transaction = new Transaction(document, "Disabling Analytical Model"))
            {
                transaction.Start();
                foreach (Element element in list)
                {
                    try
                    {
                        bool flag = element.get_Parameter(BuiltInParameter.STRUCTURAL_ANALYTICAL_MODEL).Set(0);
                    }
                    catch (Exception)
                    {
                    }
